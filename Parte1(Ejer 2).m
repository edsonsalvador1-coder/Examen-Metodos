...
lagrange = @(x,y,xp) ...
    y(1)*((xp-x(2))*(xp-x(3)))/((x(1)-x(2))*(x(1)-x(3))) + ...
    y(2)*((xp-x(1))*(xp-x(3)))/((x(2)-x(1))*(x(2)-x(3))) + ...
    y(3)*((xp-x(1))*(xp-x(2)))/((x(3)-x(1))*(x(3)-x(2)));

%% INTERPOLACIONES

% Voltaje
V41 = lagrange([37.5 40 42.5],[1.048 0.866 0.689],41);

V73 = lagrange([70 72.5 75],[0.197 0.318 0.452],73);

% Impedancia
Z41 = lagrange([37.5 40 42.5],[145.2 145.8 147.3],41);

Z73 = lagrange([70 72.5 75],[200.1 203.1 205.2],73);
...
plot(41,V41,'r*','MarkerSize',12);
plot(73,V73,'b*','MarkerSize',12);

xlabel('Frecuencia (kHz)');
ylabel('Voltaje V(f)');
title('Interpolacion de Lagrange - Voltaje');
...
plot(41,Z41,'r*','MarkerSize',12);
plot(73,Z73,'b*','MarkerSize',12);

xlabel('Frecuencia (kHz)');
ylabel('|Z| (\Omega)');
title('Interpolacion de Lagrange - Impedancia');
